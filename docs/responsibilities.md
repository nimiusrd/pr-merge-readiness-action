...
以下は今後の実装・変更の判断基準です。公開済み v0.6.0 と現在の実装には重複する判定が残っています。[現行実装との差](#現行実装との差)を参照してください。
...
CI 定義の変更をパスだけで検出して一律に対応を要求する条件は、担当者レビューを GitHub 側に設定する方針へ移します。ファイル履歴に基づく条件は、CI 定義を含めた既存ファイルに同じ基準で適用できます。
...
v0.6.0 と現在のソースには次の重複があります。以下は責務を整理する際の変更対象であり、この文書の追加によって設定契約や判定動作が変わったわけではありません。
...
現行ラベルの説明やサマリーも、実装の変更時に担当範囲へ合わせます。設定の削除を伴う変更は、対応リリースと移行手順を揃えて行います。現在の version 3 設定から必須項目を先に削除すると、設定検証で失敗します。
