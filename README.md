...
**自動ラベルには v0.5.0 以降が必要です。** workflow の `uses:` を対応リリースの配布用 SHA に更新し、default branch の設定を次のように変更します。v0.5.0 を使う場合は TOML の `action_ref` も同じ SHA に揃えます。v0.4.0 は `"auto"` を受け付けません。[移行手順](docs/workflow.md#自動ラベルへの移行)を参照してください。
...
ラベルはレビューと変更履歴の条件だけを表します。コードの差分本文は読み取らず、変更内容の妥当性・安全性やテストの十分さは評価しません。CI の実行状態と PR の open/closed・Draft・競合状態はラベルの判定に含めません。
...
たとえば Draft や競合があっても、レビュー条件を満たせばラベルは `shadow/要対応事項なし` です。必要承認数が 0 件で、変更履歴による人間の承認要求もなければ、承認なしでもこのラベルが付きます。参考 Check・レポートでは引き続き PR 状態も判定します。観測中に Draft・競合・open/closed・更新時刻だけが変わってもラベルは再観測扱いにしません。head・base・レビュー要求・レビュー内容・未解決スレッドの変化は、ラベルでも再観測が必要です。

手動同期時は、作成元とマージ先が同じリポジトリの open PR にラベルを付けます。fork PR、作成元リポジトリが削除された PR、Dependabot が作成した PR は付与対象から外し、残っている管理ラベルを除去します。Dependabot は PR 作成者が `dependabot[bot]` かで判定するため、人間による手動実行でも対象外です。closed／merged PR に残る管理ラベルも除去します。終了状態を表すラベルは付けません。

旧ラベル `shadow/レビュー条件充足`・`shadow/CI・レビュー待ち`・`shadow/要マージ判断` の付与は、自動更新では対象 PR から、手動同期では全対象 PR から除去します。旧ラベルは新規作成せず、リポジトリ内の既存のラベル定義は残します。利用側の Action をこの名称に対応したリリースの SHA に更新した後、`update-labels = true` で手動同期すると既存 PR に反映できます。
