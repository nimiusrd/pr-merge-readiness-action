...
[Action の責務](docs/responsibilities.md)と[推奨 Ruleset](docs/rulesets.md)に役割分担を記載しています。現行 v0.6.0 には承認数・未解決スレッドなどの重複判定が残っています。以下の設定・判定内容は現行版の動作を説明します。
...
現行版は承認・変更要求・未解決スレッド・変更履歴からレビュー条件を判定します。コードの差分本文は評価せず、自動マージ・自動承認は行いません。
...
以下は現行 version 3 の設定です。Ruleset との責務整理に伴う設定削除は未実施なので、必須項目を先に省略しないでください。
